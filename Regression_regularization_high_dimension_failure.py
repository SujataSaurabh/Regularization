...
lasso = Lasso(alpha=0.5) ## reduced alpha slightly for the smaller dataset size
lasso.fit(X_train, y_train)

print("--- HIGH DIMENSIONAL FAILURE DEMO ---")
print(f"Linear Regression active features: {np.sum(lr.coef_ != 0)}")
print(f"Lasso (L1) active features: {np.sum(lasso.coef_ != 0)}")
